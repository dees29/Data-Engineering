...
def create_database():
    ...
    conn = pc2.connect(dbname = "postgres", host = "localhost", port = "5432", user = "postgres", password = "Bridge****")
    conn.set_session(autocommit=True)
    cur=conn.cursor()
    
    # create sparkify database with UTF8 encoding
    cur.execute('DROP DATABASE IF EXISTS sparkifymusic')
    cur.execute("CREATE DATABASE sparkifymusic WITH ENCODING 'utf8' TEMPLATE template0")
    conn.close()
    
    # connect to sparkify database
    conn = pc2.connect(dbname="sparkifymusic", host="localhost", port = "5432", user = "postgres", password = "Bridgel@bz1")
    cur=conn.cursor()
